...
init_react();
var QueryClientContext = react_exports.createContext(
  void 0
);
var useQueryClient = (queryClient) => {
  const client = react_exports.useContext(QueryClientContext);
  if (queryClient) {
    return queryClient;
  }
  if (!client) {
...
var QueryClientProvider = ({
  ...
  return (0, jsx_runtime_exports.jsx)(QueryClientContext.Provider, { value: client, children });
};

// ../../node_modules/@tanstack/react-query/build/modern/IsRestoringProvider.js
init_react();
var IsRestoringContext = react_exports.createContext(false);
var useIsRestoring = () => react_exports.useContext(IsRestoringContext);
...
var QueryErrorResetBoundaryContext = react_exports.createContext(createValue());
var useQueryErrorResetBoundary = () => react_exports.useContext(QueryErrorResetBoundaryContext);
var QueryErrorResetBoundary = ({
  children
}) => {
  const [value] = react_exports.useState(() => createValue());
  return (0, jsx_runtime_exports.jsx)(QueryErrorResetBoundaryContext.Provider, { value, children: typeof children === "function" ? children(value) : children });
};
...
function useQueries({
  ...
  const defaultedQueries = react_exports.useMemo(
    () => queries.map((opts) => {
  ...
  const [observer] = react_exports.useState(
    () => new QueriesObserver(
      client,
      defaultedQueries,
      options
  ...
  react_exports.useSyncExternalStore(
    react_exports.useCallback(
...
var HydrationBoundary = ({
  ...
  const client = useQueryClient(queryClient);
  const optionsRef = react_exports.useRef(options);
  react_exports.useEffect(() => {
    optionsRef.current = options;
  });
  const hydrationQueue = react_exports.useMemo(() => {
    if (state) {
...
function useIsFetching(filters, queryClient) {
  const client = useQueryClient(queryClient);
  const queryCache = client.getQueryCache();
  return react_exports.useSyncExternalStore(
    react_exports.useCallback(
...
function useMutationState(options = {}, queryClient) {
  ...
  const optionsRef = react_exports.useRef(options);
  const result = react_exports.useRef(null);
...
function useMutation(options, queryClient) {
  ...
  const result = react_exports.useSyncExternalStore(
    react_exports.useCallback(
  ...
  const mutate = react_exports.useCallback(
